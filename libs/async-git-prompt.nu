...
export def async-git-prompt-refresh-cache [] {
    let cache_path = (cache-path)
    if ($cache_path != null) {
        echo (in-progress-symbol) | save $cache_path -f
        do-async $"use ($nu.config-path | path expand | path dirname)/libs/async-git-prompt.nu *; async-git-prompt-compute-sync | save ($cache_path) -f"
    }
}
